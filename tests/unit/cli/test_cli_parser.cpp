...
TEST_F(CliParserTest, EnumConversions) {
    ...
    EXPECT_EQ("console", to_string(TemplateType::Console));
    EXPECT_EQ("lib", to_string(TemplateType::Lib));
    EXPECT_EQ("gui", to_string(TemplateType::Gui));
    EXPECT_EQ("network", to_string(TemplateType::Network));

    auto templateType = to_template_type("console");
    ASSERT_TRUE(templateType.has_value());
    EXPECT_EQ(TemplateType::Console, templateType.value());

    templateType = to_template_type("invalid");
    EXPECT_FALSE(templateType.has_value());
}

TEST_F(CliParserTest, BuildSystemConversions) {
    EXPECT_EQ("cmake", to_string(BuildSystem::CMake));
    EXPECT_EQ("meson", to_string(BuildSystem::Meson));
    EXPECT_EQ("bazel", to_string(BuildSystem::Bazel));

    auto buildSystem = to_build_system("cmake");
    ASSERT_TRUE(buildSystem.has_value());
    EXPECT_EQ(BuildSystem::CMake, buildSystem.value());

    buildSystem = to_build_system("invalid");
    EXPECT_FALSE(buildSystem.has_value());
}

TEST_F(CliParserTest, PackageManagerConversions) {
    EXPECT_EQ("vcpkg", to_string(PackageManager::Vcpkg));
    EXPECT_EQ("conan", to_string(PackageManager::Conan));
    EXPECT_EQ("none", to_string(PackageManager::None));

    auto packageManager = to_package_manager("vcpkg");
...
TEST_F(CliParserTest, TestFrameworkConversions) {
    EXPECT_EQ("gtest", to_string(TestFramework::GTest));
    EXPECT_EQ("catch2", to_string(TestFramework::Catch2));
    EXPECT_EQ("doctest", to_string(TestFramework::Doctest));

    auto testFramework = to_test_framework("gtest");
...
TEST_F(CliParserTest, LanguageConversions) {
    EXPECT_EQ("en", to_string(Language::English));
    EXPECT_EQ("zh", to_string(Language::Chinese));
    EXPECT_EQ("es", to_string(Language::Spanish));

    auto language = to_language("en");
...
TEST_F(CliParserTest, AllEnumValues) {
    ...
    auto templateTypes = all_template_types();
    ...
    auto buildSystems = all_build_systems();
    ...
    auto packageManagers = all_package_managers();
