...
TEST_F(TerminalUtilsTest, WrapText) {
    std::string longText =
            "This is a very long line of text that should be wrapped at a certain width to test "
            "the text wrapping functionality.";
...
TEST_F(TerminalUtilsTest, GetBorderChars) {
    ...
    EXPECT_EQ(singleBorder.size(),
              11);  // Should have 11 border characters (corners, edges, junctions)
...
TEST_F(TerminalUtilsTest, ShowBox) {
    ...
    EXPECT_NO_THROW(
            TerminalUtils::showBox(lines, BorderStyle::Single, Color::Reset, Color::Reset, ""));
}

TEST_F(TerminalUtilsTest, ShowBoxWithStyle) {
    std::vector<std::string> lines = {"Styled Box", "With custom colors"};
    EXPECT_NO_THROW(
            TerminalUtils::showBox(lines, BorderStyle::Double, Color::Blue, Color::White, "Title"));
}
...
TEST_F(TerminalUtilsTest, ShowTwoColumnLayout) {
    std::vector<std::pair<std::string, std::string>> items = {
            {"Key 1", "Value 1"}, {"Key 2", "Value 2"}, {"Longer Key", "Longer Value"}};
    EXPECT_NO_THROW(TerminalUtils::showTwoColumnLayout(items));
}

TEST_F(TerminalUtilsTest, ShowFeatureList) {
    std::vector<std::pair<std::string, std::string>> features = {
            {"Feature 1", "Description of feature 1"}, {"Feature 2", "Description of feature 2"}};
...
TEST_F(TerminalUtilsTest, ColorEnumValues) {
    ...
    std::vector<Color> colors = {Color::Reset,        Color::Black,      Color::Red,
...
TEST_F(TerminalUtilsTest, TextStyleEnumValues) {
    std::vector<TextStyle> styles = {TextStyle::Bold,      TextStyle::Dim,   TextStyle::Italic,
...
TEST_F(TerminalUtilsTest, BorderStyleEnumValues) {
    std::vector<BorderStyle> styles = {BorderStyle::Single, BorderStyle::Double,
                                       BorderStyle::Rounded, BorderStyle::Bold, BorderStyle::Ascii};
