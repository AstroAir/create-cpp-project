...
static void BM_Add(benchmark::State& state) {
  // 这段代码会被计时
  for (auto _ : state) {
    benchmark::DoNotOptimize(test-premake-project::add(42, 24));
  }
}
BENCHMARK(BM_Add);
...
static void BM_Add_WithArgs(benchmark::State& state) {
  ...
  for (auto _ : state) {
    benchmark::DoNotOptimize(test-premake-project::add(a, b));
  }
}
// 测试不同输入组合
...
static void BM_ExampleGetValue(benchmark::State& state) {
  test-premake-project::Example example(state.range(0));
  for (auto _ : state) {
