...
bool TemplateManager::createProject(const CliOptions& options) {
    try {
        ...
        if (options.sourceType == SourceType::Template) {
            ...
            // Find corresponding template factory
            auto it = templateFactories_.find(templateTypeStr);
            if (it == templateFactories_.end()) {
                std::cerr << "Error: Unknown project template type '" << templateTypeStr << "'.\n";
                return false;
            }
        ...
        std::cerr << "错误: 创建项目时发生异�? " << e.what() << "\n";
        return false;
    }
}

std::unique_ptr<TemplateBase> TemplateManager::createFrameworkTemplate(
        const CliOptions& options, utils::FrameworkIntegration::Framework framework) {
    ...
    utils::FrameworkIntegration::installFramework(
            framework, projectPath, utils::FrameworkIntegration::PackageManager::vcpkg);
