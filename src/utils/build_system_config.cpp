...
BuildSystemConfig BuildSystemConfigManager::createDefaultConfig(BuildSystem buildSystem, TemplateType templateType) {
...
BuildSystemConfig BuildSystemConfigManager::configureInteractively(const CliOptions& options) {
    ...
    // Advanced options
    std::vector<std::string> advancedOptions = {
    ...
    auto selectedOptions = TerminalUtils::showMultiSelectDialog(
        "Select advanced build options", advancedOptions, defaultSelected);
    
    // Apply selected options
    config.compilerSettings.enableLTO = std::find(selectedOptions.begin(), selectedOptions.end(), 
                                                  "Enable Link-Time Optimization (LTO)") != selectedOptions.end();
    config.compilerSettings.enablePCH = std::find(selectedOptions.begin(), selectedOptions.end(), 
    ...
    config.compilerSettings.enableClangTidy = std::find(selectedOptions.begin(), selectedOptions.end(), 
...
OptimizationLevel BuildSystemConfigManager::selectOptimizationLevel(OptimizationLevel defaultLevel) {
    std::vector<std::string> levels = {
        "None (-O0) - No optimization, fastest compilation",
        "Debug (-Og) - Optimize for debugging",
        "Size (-Os) - Optimize for size",
        "Speed (-O2) - Optimize for speed",
    ...
    int selected = TerminalUtils::showInteractiveMenu(
        levels, "Select optimization level", defaultIndex);
...
std::string BuildSystemConfigManager::generateCMakeConfig(const BuildSystemConfig& config, const std::string& projectName) {
...
std::string BuildSystemConfigManager::generateMesonConfig(const BuildSystemConfig& config, const std::string& projectName) {
...
std::string BuildSystemConfigManager::cppStandardToString(CppStandard standard) {
    switch (standard) {
        case CppStandard::Cpp11: return "11";
        case CppStandard::Cpp14: return "14";
        case CppStandard::Cpp17: return "17";
        case CppStandard::Cpp20: return "20";
        case CppStandard::Cpp23: return "23";
        case CppStandard::Latest: return "23";
        default: return "17";
    }
}
...
std::string BuildSystemConfigManager::buildConfigurationToString(BuildConfiguration config) {
    switch (config) {
        ...
        case BuildConfiguration::Custom: return "Custom";
