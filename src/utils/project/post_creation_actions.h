...
enum class ActionResult { Success, Warning, Error, Skipped };
...
class PostCreationActions {
   public:
    ...
    void setSkipOptional(bool skip) {
        skipOptional_ = skip;
    }
    ...
    // Results
    std::vector<ActionStatus> getResults() const {
        return results_;
    }
   ...
   private:
    ...
    // Helper methods
    ActionStatus executeAction(const std::string& name, const std::string& description,
