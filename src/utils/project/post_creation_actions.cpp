#include "../project/post_creation_actions.h"
...
#include "../build/dependency_manager.h"
    ...
    for (const auto& [name, action] : actions) {
        ...
        switch (status.result) {
            case ActionResult::Success:
                TerminalUtils::showSuccess(
                        fmt::format("�?{} completed successfully", status.name));
                break;
            ...
            case ActionResult::Error:
                TerminalUtils::showError(
                        fmt::format("�?{} failed: {}", status.name, status.message));
                if (!skipOptional_) {
                    return false;
                }
                break;
            case ActionResult::Skipped:
                TerminalUtils::showInfo(
                        fmt::format("⏭️  {} skipped: {}", status.name, status.message));
                break;
        }
    }
...
ActionStatus PostCreationActions::validateProjectStructure(const CliOptions& options) {
    return executeAction("Project Validation", "Validating project structure and files", [&]() {
...
ActionStatus PostCreationActions::installDependencies(const CliOptions& options) {
    return executeAction("Dependency Installation", "Installing project dependencies", [&]() {
...
ActionStatus PostCreationActions::runInitialBuild(const CliOptions& options) {
    return executeAction("Initial Build", "Building the project for the first time", [&]() {
        std::string projectPath = options.projectName;
...
ActionStatus PostCreationActions::runInitialTests(const CliOptions& options) {
    return executeAction("Initial Tests", "Running initial test suite", [&]() {
...
ActionStatus PostCreationActions::setupDevelopmentEnvironment(const CliOptions& options) {
    return executeAction("Development Environment", "Setting up IDE configurations", [&]() {
        std::string projectPath = options.projectName;
        bool success = true;
...
ActionStatus PostCreationActions::createInitialCommit(const CliOptions& options) {
    return executeAction("Initial Git Commit", "Creating initial Git commit", [&]() {
        if (!options.initGit) {
...
bool PostCreationActions::validateFile(const std::string& path,
...
void PostCreationActions::printSummary() const {
    ...
    if (warnings > 0)
        std::cout << fmt::format("⚠️  Warnings: {}\n", warnings);
    if (errors > 0)
        std::cout << fmt::format("�?Errors: {}\n", errors);
    if (skipped > 0)
        std::cout << fmt::format("⏭️  Skipped: {}\n", skipped);

    std::cout << "\n";
}

bool PostCreationActions::hasErrors() const {
    return std::any_of(results_.begin(), results_.end(), [](const ActionStatus& status) {
