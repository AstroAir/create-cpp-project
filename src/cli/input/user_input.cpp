...
UserInput::ValidatorFunction UserInput::numericValidator() {
  return [](const std::string &input) -> ValidatorResult {
    if (input.empty()) return "Input cannot be empty";
...
UserInput::ValidatorFunction
UserInput::oneOfValidator(const std::vector<std::string_view> &validValues) {
...
UserInput::readChoice(std::string_view prompt,
  ...
  while (true) {
    ...
    fmt::print(fmt::fg(fmt::color::red), "Invalid selection, please try again.\n");
  }
}
...
std::string UserInput::readChoiceWithStyle(
  ...
  while (true) {
    std::string input = readWithHighlight("Please select", defaultValue, utils::Color::BrightYellow);
    ...
    std::cout << TerminalUtils::colorize("Invalid selection, please try again.", utils::Color::BrightRed) << "\n";
  }
}
