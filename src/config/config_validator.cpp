...
ValidationResult ConfigValidator::validateConfiguration(const CliOptions& options) const {
    ...
    result.messages.insert(result.messages.end(), nameResult.messages.begin(), nameResult.messages.end());
    if (!nameResult.isValid) result.isValid = false;

    // Validate template compatibility
    auto templateResult = validateTemplateCompatibility(options);
    result.messages.insert(result.messages.end(), templateResult.messages.begin(), templateResult.messages.end());
    if (!templateResult.isValid) result.isValid = false;

    // Validate build system compatibility
    auto buildResult = validateBuildSystemCompatibility(options);
    result.messages.insert(result.messages.end(), buildResult.messages.begin(), buildResult.messages.end());
    if (!buildResult.isValid) result.isValid = false;

    // Validate package manager compatibility
    auto packageResult = validatePackageManagerCompatibility(options);
    result.messages.insert(result.messages.end(), packageResult.messages.begin(), packageResult.messages.end());
    if (!packageResult.isValid) result.isValid = false;

    // Platform-specific validation
    if (platformSpecificValidation_) {
        auto platformResult = validatePlatformCompatibility(options);
        result.messages.insert(result.messages.end(), platformResult.messages.begin(), platformResult.messages.end());
        if (!platformResult.isValid) result.isValid = false;
    }

    // Run custom validation rules
    for (const auto& [name, rule] : customRules_) {
        auto ruleResult = rule(options);
        result.messages.insert(result.messages.end(), ruleResult.messages.begin(), ruleResult.messages.end());
...
ValidationResult ConfigValidator::validateTemplateCompatibility(const CliOptions& options) const {
    ...
    if (!buildCompat.isCompatible) {
        result.addMessage(ValidationSeverity::Error, "Template Compatibility",
    ...
    auto packageCompat = checkTemplateWithPackageManager(options.templateType, options.packageManager);
...
ValidationResult ConfigValidator::validateBuildSystemCompatibility(const CliOptions& options) const {
    ValidationResult result;
...
ValidationResult ConfigValidator::validatePackageManagerCompatibility(const CliOptions& options) const {
    ValidationResult result;
    ...
    if (options.packageManager != PackageManager::None) {
        if (!checkPackageManagerAvailability(options.packageManager)) {
            result.addMessage(ValidationSeverity::Warning, "Package Manager",
                             ...
                             "Install " + std::string(enums::to_string(options.packageManager)) + " or use --package none");
        }
    }
...
bool ConfigValidator::isReservedKeyword(const std::string& name) const {
    static const std::vector<std::string> reserved = {
        ...
        "makefile", "readme", "license", "copyright", "version", "config",
...
CompatibilityInfo ConfigValidator::checkTemplateWithBuildSystem(TemplateType template_type, BuildSystem build_system) const {
...
CompatibilityInfo ConfigValidator::checkTemplateWithPackageManager(TemplateType template_type, PackageManager package_manager) const {
    CompatibilityInfo info;
    info.isCompatible = true; // Default to compatible
...
CompatibilityInfo ConfigValidator::checkBuildSystemWithPackageManager(BuildSystem build_system, PackageManager package_manager) const {
    CompatibilityInfo info;
    info.isCompatible = true; // Default to compatible
...
CompatibilityInfo ConfigValidator::checkTestFrameworkWithBuildSystem(TestFramework test_framework, BuildSystem build_system) const {
    CompatibilityInfo info;
    info.isCompatible = true; // Default to compatible
