...
class EditorConfigManager {
   public:
    // 为指定的编辑器创建配置文件
    static bool createEditorConfig(const std::string& projectPath, const std::string& editorType,
   ...
   private:
    // 创建VS Code配置
    static bool createVSCodeConfig(const std::string& projectPath, const CliOptions& options);

    // 创建CLion配置
    static bool createCLionConfig(const std::string& projectPath, const CliOptions& options);

    // 创建Visual Studio配置
    static bool createVisualStudioConfig(const std::string& projectPath, const CliOptions& options);
