...
public:
    ...
    std::vector<ConfigEntry> getConfigHierarchy(const std::string& key);
    ...
    std::vector<ConfigEntry> getKeysInCategory(const std::string& category);
...
private:
    ...
    std::map<std::string, ConfigEntry> configEntries_;
    ...
    void registerConfigEntry(const ConfigEntry& entry);
    ...
    // Validation helpers
    bool validateConfigEntry(const ConfigEntry& entry);
...
namespace config_utils {
    ...
    nlohmann::json promptForValue(const std::string& key, const ConfigEntry& entry);
